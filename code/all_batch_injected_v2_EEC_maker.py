# modified from all_batch_EEC_maker.py to include injected v2
# similar method to injected_v2_inclusive_EEC_maker.ppy
...
def main():
    ...
    parser.add_argument("--v2", required=True, type=str, help="v2 amount to inject (pass as a string)")
    ...
    for data in uproot.iterate(tree_paths, expressions=["genJetPt", "genJetEta", "genDau_pt", "genDau_eta", "genDau_phi", "genDau_chg"], step_size="150 MB"):
        ...
        for bin_key in bin_pass_dict.keys():
            ...
            psi_vals = np.random.uniform(-np.pi, np.pi, size=num_jets_in_chunk) # for v2
    ...
    full_output_path = os.path.join(out_dir, f"EEC_Output_Batch{args.batchnum}.root")
